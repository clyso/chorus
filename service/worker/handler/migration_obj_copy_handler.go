...
func (s *svc) HandleMigrationObjCopyHead(ctx context.Context, task *asynq.Task) error {
	...
	release, refresh, err := s.locker.Lock(ctx, lock.ObjKey(payload.ToStorage, domObj))
	if err != nil {
		return fmt.Errorf("unable to make object lock: %w", err)
	}
	defer release()

	if err := s.process(ctx, logger, payload, refresh); err != nil {
...
func (s *svc) process(ctx context.Context, logger *zerolog.Logger, payload tasks.MigrateObjCopyPayload, refresh func(time.Duration) error) error {
	domObj := dom.Object{
		Bucket:  payload.Bucket,
		Name:    payload.Obj.Name,
		Version: payload.Obj.VersionID,
